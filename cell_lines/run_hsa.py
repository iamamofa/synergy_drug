...
for cline in ["BxPC-3", "MiaPaca2", "Panc1"]:
    ...
    syn.to_csv("results/hsa/%s.csv"%cline, index=None)
    ...
    for fmt in ['pdf', 'png']:
        # Plot HSA heatmaps
        plots.plot_heatmap(X1, X2, Z, fname="results/hsa/%s.%s"%(cline,fmt),
            xlabel="Gemcitabine (μM)", ylabel="MSeA (μM)", title="%s HSA synergy "%cline,
            center_on_zero=True, figsize=figsize, vmin=vmin, vmax=vmax
        )

        # Plot HSA pvals
        plots.plot_heatmap(X1, X2, -np.log10(hsa_pval), fname="results/hsa/%s_pvals.%s"%(cline,fmt),
            xlabel="Gemcitabine (μM)", ylabel="MSeA (μM)", title="HSA (-log10(p-val))",
